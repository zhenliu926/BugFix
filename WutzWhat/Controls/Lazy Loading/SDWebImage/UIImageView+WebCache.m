...
{
    __weak UIImage *PH = [UIImage imageNamed:@"list_thumbnail.png"];
    UIGraphicsBeginImageContext(CGSizeMake(PH.size.width, PH.size.height-1));
    
    [PH drawAtPoint:CGPointMake(0, -1)];
    
    PH = UIGraphicsGetImageFromCurrentImageContext();
    
    UIGraphicsEndImageContext();

    [self setImageWithURL:url placeholderImage:PH];
}

- (void)setImageWithURLWithOutPlaceHolder:(NSURL *)url
{
