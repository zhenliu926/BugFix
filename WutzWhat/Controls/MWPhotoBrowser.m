...
- (void)toggleControlsForVideo:(NSNumber *)isVideo;
...
-(void)forcePortraitOrientation
{
    ...
    {        
        DummyPortraitViewController *controller = [[DummyPortraitViewController alloc] init];
...
-(void)oneFingerSwipeUporDown:(id)sender
{
...
- (void)viewDidLoad {
    ...
    [[[UISwipeGestureRecognizer alloc] initWithTarget:self action:@selector(oneFingerSwipeUporDown:)] autorelease];
    ...
    UITapGestureRecognizer *doubleTapGestureRecognizer = [[UITapGestureRecognizer alloc]
                                                          initWithTarget:self
                                                          action:@selector(oneFingerSwipeUporDown:)];
...
- (MWZoomingScrollView *)dequeueRecycledPage {
	MWZoomingScrollView *page = nil;//[_recycledPages anyObject];
	if (page) {
...
- (void)hideControlsAfterDelay {
	if (![self areControlsHidden]) {
        [self cancelControlHiding];
		_controlVisibilityTimer = [[NSTimer scheduledTimerWithTimeInterval:1 target:self selector:@selector(hideControls) userInfo:nil repeats:NO] retain];
	}
}
...
- (void)toggleControlsForVideo:(NSNumber *)isVideo
{
    id <MWPhoto> photo = [_photos objectAtIndex:_currentPageIndex];
    
    if ([isVideo boolValue])
    {
        NSLog(@"is Video Object... and url is : %@", [photo videoURL]);
        [self playVideoWithURL:[photo videoURL]];
    }
    else
    {
         NSLog(@"is image Object...");
        [self forcePortraitOrientation];
    }
}
...
-(void)playVideoWithURL:(NSURL *)videoURL
{
    ...
    [self.moviePlayer playVideoFromURL:videoURL containerView:self];
}
