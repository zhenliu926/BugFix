...
//        
//        UIPinchGestureRecognizer *pinchGesture = [[UIPinchGestureRecognizer alloc] initWithTarget:self action:nil];
//        pinchGesture.enabled = NO;
//        
//        [super addGestureRecognizer:pinchGesture];
...
- (void)scrollViewWillBeginZooming:(UIScrollView *)scrollView withView:(UIView *)view {
    if ([_photo isVideo]) [scrollView setScrollEnabled:NO];
	[_photoBrowser cancelControlHiding];
}
...
- (void)handleSingleTap:(CGPoint)touchPoint {
	[_photoBrowser performSelector:@selector(toggleControlsForVideo:) withObject:[NSNumber numberWithBool:[_photo isVideo]] afterDelay:0.2];
}
...
- (void)handleDoubleTap:(CGPoint)touchPoint {
    ...
    return; // Asad Ali// return here to exit. 
...
{
    if (([gestureRecognizer isKindOfClass:[UIPinchGestureRecognizer class]] || [gestureRecognizer isKindOfClass:[UIPanGestureRecognizer class]]))
    {        
        gestureRecognizer.enabled = ![_photo isVideo];
    }
